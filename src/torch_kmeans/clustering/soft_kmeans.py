...
class SoftKMeans(KMeans):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.distance.is_inverted:
            raise ValueError("soft k-means requires inverted distance measure (i.e. similarity).")
    ...
    def _cluster(
        ...
        # Precompute tensor expansions
        k_mask_expanded = k_mask.unsqueeze(1).expand(-1, self.num_init, -1)
        k_mask_bcast = k_mask.unsqueeze(1).unsqueeze(2).expand(-1, self.num_init, n, -1)
        
        with torch.no_grad():
            for i in range(self.max_iter):
                centers[k_mask_expanded] = 0
        ...
        if self.num_init > 1:
            ...
            k_mask_expanded = k_mask.unsqueeze(1).expand(-1, 1, -1)
            k_mask_bcast = k_mask.unsqueeze(1).unsqueeze(2).expand(-1, 1, n, -1)
    ...
    def _cluster_iter(self, x: Tensor, centers: Tensor) -> Tensor:
        ...
        dist = self._pairwise_distance(x, centers)
        dist[dist == 0] = float("-inf")  # Efficient masking
        c_assign = torch.softmax(self.temp * dist, dim=-1)
        per_cluster = c_assign.sum(dim=-2)
        cluster_mean = (
            c_assign.permute(0, 1, 3, 2).unsqueeze(-1) @ x.unsqueeze(1).unsqueeze(2)  # Optimized matrix multiplication
        ).squeeze(-2)
        centers = torch.diag_embed(1.0 / (per_cluster + self.eps)) @ cluster_mean  # Optimized matrix multiplication
        centers[dist.any(dim=-2)] = 0  # Efficient masking
        return centers
